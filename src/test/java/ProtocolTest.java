...
class ProtocolTest {
    ...
    private static final byte[] AES_KEY = "0123456789abcdef".getBytes(); // 16 bytes = AES-128

    private byte[] buildPacket(byte bSrc, long bPktId, int cType, int bUserId, byte[] body, byte[] key) throws Exception {
    ...
    void receivePacket_emptyBody_parsesCorrectly() throws Exception {
        ...
        assertEquals(0, payload.message.message.length);
    }
    ...
    void receivePacket_wLen_matchesBodySize() throws Exception {
        ...
        assertEquals(28, payload.wLen); // 8 (cType + bUserId) + 20 (body)
    }
    ...
    void sendReceive_withEncryption_roundTrip() throws Exception {
        ...
        ProtocolPayload payload = new ProtocolParser().receivePacket(raw);
        byte[] decrypted = payload.message.decrypt(AES_KEY);

        assertArrayEquals(body, decrypted);
    }
